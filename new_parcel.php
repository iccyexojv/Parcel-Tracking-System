        ...
        <table class="table table-bordered" id="parcel-items">
          ...
          <tbody>
            <tr>
              ...
              <td><input type="number" name='height[]' value="<?php echo isset($height) ? $height :'' ?>" required></td>
              <td><input type="number" name='length[]' value="<?php echo isset($length) ? $length :'' ?>" required></td>
              <td><input type="number" name='width[]' value="<?php echo isset($width) ? $width :'' ?>" required></td>
              <td><input type="number" class="text-right number" name='price[]' value="<?php echo isset($price) ? $price :'' ?>" required></td>
              <?php if(!isset($id)): ?>
...
<div id="ptr_clone" class="d-none">
  <table>
    <tr>
        ...
        <td><input type="number" class="text-right number" name='price[]' required></td>
        <td><button class="btn btn-sm btn-danger" type="button" onclick="$(this).closest('tr').remove() && calc()"><i class="fa fa-times"></i></button></td>
      </tr>
  </table>
</div>
...
<script>
function autoCalculatePrice(row) {
    let weight = parseFloat($(row).find('[name="weight[]"]').val()) || 0;
    let height = parseFloat($(row).find('[name="height[]"]').val()) || 0;
    let length = parseFloat($(row).find('[name="length[]"]').val()) || 0;
    let width  = parseFloat($(row).find('[name="width[]"]').val()) || 0;

    // Parameters (customize as needed)
    const volumeDivisor = 5000;
    const ratePerKg = 10;

    let volumeWeight = (length * width * height) / volumeDivisor;
    let weightCharge = weight * ratePerKg;

    let price = Math.max(volumeWeight * ratePerKg, weightCharge);
    price = isNaN(price) ? 0 : price.toFixed(2);

    $(row).find('[name="price[]"]').val(parseFloat(price).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}));

    calc(); // Update total
}

// Attach auto-calc to each input field
function bindAutoCalcEvents(row) {
    $(row).find('input').on('input', function () {
        autoCalculatePrice(row);
    });
}
...
$(document).ready(function () {
    $('#parcel-items tbody tr').each(function () {
        bindAutoCalcEvents(this);
        autoCalculatePrice(this);
    });

    $('#new_parcel').click(function () {
        var tr = $('#ptr_clone tr').clone();
        $('#parcel-items tbody').append(tr);
        bindAutoCalcEvents(tr);
        autoCalculatePrice(tr);
    });
});
</script>
